...
public class Green : MonoBehaviour
{
    ...
    void Attacks()
    {
        ...
        if (Input.GetKeyDown(AttackKey) && _AttackTime <= 0 && isAttacking == false)
        {
            ...
            {
                GameObject razorleaf = Instantiate(AttackCollider, AttackSpawnPoint.position, AttackSpawnPoint.rotation);
                Rigidbody2D rb = razorleaf.GetComponent<Rigidbody2D>();
                if (rb != null)
                {
                    //rb.velocity = razorleafSpawnPoint.up * 0.5f;
    ...
    void AttackSkill()
    {
        if (Input.GetKeyDown(AttackSkillKey) && _AttackSkillTime <= 0 && isAttackingSkill == false)
        {
            ...
            {
                GameObject LeafCyclone = Instantiate(SkillAttackCollider, SkillAttackSpawnPoint.position, SkillAttackSpawnPoint.rotation);
                Rigidbody2D rb = LeafCyclone.GetComponent<Rigidbody2D>();
                if (rb != null)
                {
                    //rb.velocity = SkillAttackSpawnPoint.up * 0.5f;
                    Vector2 direction = new Vector2(-transform.localScale.x, 0);
